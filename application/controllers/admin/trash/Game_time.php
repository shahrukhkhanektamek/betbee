...
class Game_time extends CI_Controller {

   protected $arr_values = array(
						   	'title'=>'Game Time', 
						   	'table_name'=>'game_times',
						   	'page_title'=>'Game Time',
						   	"submit_url"=>panel.'/game_time/update',
						   	"folder_name"=>'game_time',
						   	"upload_path"=>'upload/',
						   	"back_btn"=>panel.'/game_time',
						   	"btn_url"=>panel.'/game_time/add',
						   	"add_btn_url"=>panel.'/game_time/add',
						   	"edit_btn_url"=>panel.'/game_time/edit/',
						   	"view_btn_url"=>panel.'/game_time/view/',
						   	"controller_name"=>'game_time',
						   	"page_name"=>'game_time-detail.php',
						   	"keys"=>'game_times.id,game_times.name',
   ...
   public function __construct()
	...
	public function load_data()
	{
		...
		$where_query .= " game_times.status='$status' and game_times.is_delete='$is_delete' and game_times.game_id='$game_id' ";
		...
		$this->db
			->select("game_times.id as id")
			->select("game.name as game_name")
			->select("game_times.open_time as open_time")
			->select("game_times.close_time as close_time")
			->select("game_times.status as status")
			->select("game_times.name as name")
			->select("game_times.times as times")
			->join("game as game","game.id=game_times.game_id","LEFT");
	...
	public function update($id="")
	{
      ...
      $old_slug = '';
	   ...
	   $user_data = array(
			...
			"times"=>json_encode($this->input->post('days')),
			"status"=>$status,
	   );			   
