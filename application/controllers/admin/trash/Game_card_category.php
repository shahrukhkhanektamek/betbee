...
class Game_card_category extends CI_Controller {

   protected $arr_values = array(
						   	'title'=>'Game card category', 
						   	'table_name'=>'game_card_category',
						   	'page_title'=>'Game card category',
						   	"submit_url"=>panel.'/game_card_category/update',
						   	"folder_name"=>'game_card_category',
						   	"upload_path"=>'upload/',
						   	"back_btn"=>panel.'/game_card_category',
						   	"btn_url"=>panel.'/game_card_category/add',
						   	"add_btn_url"=>panel.'/game_card_category/add',
						   	"edit_btn_url"=>panel.'/game_card_category/edit/',
						   	"view_btn_url"=>panel.'/game_card_category/view/',
						   	"controller_name"=>'game_card_category',
						   	"page_name"=>'game_card_category-detail.php',
						   	"keys"=>'game_card_category.id,game_card_category.name',
   ...
   public function __construct()
	...
	public function load_data()
	{
		...
		$keys = '';
		$where_query = "";
		$order_by = "game_card_category.id desc";
		$is_delete = 0;
		$game_id = 0;
		...
		$this->db
			->select("game_card_category.id as id")
			->select("game.name as game_name")
			->select("game_card_category.status as status")
			->select("game_card_category.name as name")
			->select("game_card_category.category_card_id as category_card_id")
			->join("game as game","game.id=game_card_category.game_id","LEFT");
	...
	public function edit($id="",$game_id='')
	{
	...
	public function view($id="")
	{
		...
		if(!empty($row))
		{
			...
			$data['game_availability'] = $this->db->get_where("game_availability",array("vendor_id"=>$row[0]->vendor_id,))->result_object();
	...
	public function update($id="")
	{
      ...
      $old_slug = '';
	   ...
	   $user_data = array(
			...
			"category_card_id"=>$this->input->post('category_card_id'),
			"status"=>$status,
	   );			   
