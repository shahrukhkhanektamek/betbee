                           ...
                           <div class="card-body">
                              ...
                              <form class="form_data row" method="post" enctype="multipart/form-data" action="<?=base_url($submit_url) ?>" id="form1" novalidate>
                                 ...
                                 <div class="form-group col-xl-6">
                                  ...
                                  <div class="form-group col-xl-6">
                                      <label>Select Game *</label>
                                      <select class="form-control select2" name="category_card_id" required>
                                          <option value="">Select</option>
                                          <?php 
                                  ...
                                  <div class="form-group col-xl-6">
                                      ...
                                      <select class="form-control select2" name="card_id">
                                          <option value="">Select</option>
                                          <?php 
                                          $users = $this->db->get_where("card",array("status"=>1,"is_delete"=>0,))->result_object();
                                          foreach ($users as $key => $value) {
                                            $selected = '';
                                            if(!empty($row))
                                               if($row->card_id==$value->id) $selected = 'selected';
                                          ?>
                                            <option <?=$selected ?> value="<?=$value->id ?>"><?=$value->name ?></option>
                                          <?php } ?>
                                      </select>
