...
class Starline_declare_result extends CI_Controller {

   protected $arr_values = array(
						   	'title'=>'Starline Declare Result', 
						   	'table_name'=>'game_bet',
						   	'page_title'=>'Starline Declare Result',
						   	"submit_url"=>panel.'/starline_declare_result/update',
						   	"folder_name"=>'starline_declare_result',
						   	"upload_path"=>'upload/',
						   	"back_btn"=>panel.'/starline_declare_result',
						   	"btn_url"=>panel.'/starline_declare_result/add',
						   	"add_btn_url"=>panel.'/starline_declare_result/add',
						   	"edit_btn_url"=>panel.'/starline_declare_result/edit/',
						   	"plan_btn_url"=>panel.'/booking/index/',
						   	"view_btn_url"=>panel.'/starline_declare_result/view/',
						   	"controller_name"=>'starline_declare_result',
						   	"page_name"=>'starline_declare_result-detail.php',
   ...
   public function __construct()
    ...
    else if(!empty($check_data) && $data['session_id']==2)
    {
		...
		$open_digit_win_number = '*';
		$game_time = $this->db->select('open_time,close_time')->get_where("game_times",array("id"=>$data['time_id'],))->result_object();
		$card = $this->db->select('name')->get_where("game",array("id"=>$data['game_id'],))->result_object();
		$game_name = $card[0]->name;
		if($data['session_id']==1) $game_name .= date("h:i A",strtotime($game_time[0]->open_time));
		if($data['session_id']==2) $game_name .= date("h:i A",strtotime($game_time[0]->close_time));
		if(trim($data['open_panna_win_number'])!='') $open_panna_win_number = $data['open_panna_win_number'];
		if(trim($data['open_digit_win_number'])!='') $open_digit_win_number = $data['open_digit_win_number'];
		$title = $open_panna_win_number.'-'.$open_digit_win_number;
	...
	public function about_to_win_list($table_name)
	{
      ...
      else
      {
        ...
        $jodi_digit_number = $check_data->open_single_number.$close_digit_win_number;
        $bid2 = $panna;
